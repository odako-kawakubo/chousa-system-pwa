 ...
 *
 * 案件管理の正式入口。
 * 案件切替／Firestore読込／リアルタイム購読を同じ経路で扱う。
 */
...
function updateProjectSyncCursors(projectId, cursors = {}, { completed = false, source = 'unspecified' } = {}) {
  ...
  syncDiagnosticLog('SYNC_CURSOR_BEFORE', { projectId, source, current, incoming: normalizeRecordCursors(cursors) });
...
function applyProjectRecordChanges(project, changes = []) {
  ...
  const unsentEntries = listUnsent({ projectId: project.projectId });
  const unsentKeys = new Set(unsentEntries.map((item) => `${item.recordType}|${item.recordId}`));
  const safeChanges = [];
  let skipped = 0;

  syncDiagnosticLog('SYNC_APPLY_START', {
    projectId: project.projectId,
    incoming: changes.map((change) => ({ recordType: change.recordType, changeType: change.changeType, id: String(change.id || '') })),
    materialIdsBefore: materialRecordStore.exportSnapshot().map((record) => String(record.materialId || '')).filter(Boolean),
  ...
  changes.forEach((change) => {
    ...
    if (unsentKeys.has(key)) {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: change.recordType, recordId: String(change.id || ''), result: 'skipped', reason: 'unsent' });
      return;
    }
  ...
  if (materialChanges.length) {
    ...
    materialChanges.forEach((change) => {
      ...
      if (!id) {
        skipped += 1;
        syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'material', recordId: '', result: 'skipped', reason: 'invalidRecord' });
        return;
      }
      const current = materialRecordStore.get(id);
      if (change.changeType !== 'removed' && current && sameFieldEditedAt(current.fieldEditedAt, change.record?.fieldEditedAt)) {
        skipped += 1;
        syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'material', recordId: id, result: 'skipped', reason: 'sameFieldEditedAt' });
        return;
      }
      ...
      changed = true;
      applied += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'material', recordId: id, result: 'applied', changeType: change.changeType });
    });
    if (materialChanged) {
  ...
  safeChanges.filter((item) => item.recordType === 'finish').forEach((change) => {
    ...
    if (!id) {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'finish', recordId: '', result: 'skipped', reason: 'invalidRecord' });
      return;
    }
    const current = finishRecordStore.get(id);
    if (change.changeType !== 'removed' && current && sameFieldEditedAt(current.fieldEditedAt, change.record?.fieldEditedAt)) {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'finish', recordId: id, result: 'skipped', reason: 'sameFieldEditedAt' });
      return;
    }
    applyKnownFinishChange(project.projectId, change);
    finishChanged = true;
    applied += 1;
    syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'finish', recordId: id, result: 'applied', changeType: change.changeType });
  });
  if (finishChanged) {
  ...
  safeChanges.filter((item) => item.recordType === 'photo').forEach((change) => {
    ...
    if (!id) {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'photo', recordId: '', result: 'skipped', reason: 'invalidRecord' });
      return;
    }
    const current = photoRecordStore.get(id);
    if (change.changeType !== 'removed' && current && sameFieldEditedAt(current.fieldEditedAt, change.record?.fieldEditedAt)) {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'photo', recordId: id, result: 'skipped', reason: 'sameFieldEditedAt' });
      return;
    }
    if (change.changeType === 'removed') {
      ...
      changed = true;
      applied += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'photo', recordId: id, result: 'applied', changeType: 'removed' });
      return;
    }
    ...
    if (normalized) {
      ...
      changed = true;
      applied += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'photo', recordId: id, result: 'applied', changeType: change.changeType });
    } else {
      skipped += 1;
      syncDiagnosticLog('SYNC_APPLY_CHANGE', { projectId: project.projectId, recordType: 'photo', recordId: id, result: 'skipped', reason: 'invalidRecord' });
    }
  });
  ...
  syncDiagnosticLog('SYNC_STORE_AFTER', {
    ...
    materialIds: materialRecordStore.exportSnapshot().map((record) => String(record.materialId || '')).filter(Boolean)
  });
...
async function openFirestoreProjectSession(target) {
  ...
  syncDiagnosticLog('SYNC_BASELINE_CHECK', {
    ...
    storedCursors,
    finishChangeCursor,
    reason: useLocalSnapshot ? 'sync-history-and-finish-baseline' : (!hasSyncHistory ? 'no-sync-history' : 'no-finish-baseline')
  });
  ...
  try {
    ...
    updateProjectSyncCursors(project.projectId, caughtUpCursors, { completed: true, source: 'catchup' });
    ...
    const stop = subscribeRealtimeProjectRecordsForProject(project, {
      ...
      onState: ({ recordType, fromCache }) => {
        ...
        if (serverReadyTypes.size === 3) markReady(latestCursorValue(getProjectRecordCursors(project.projectId)));
      },
      onChanges: (changes) => {
        ...
        try {
          ...
          syncDiagnosticLog('SYNC_LISTENER_APPLY_RESULT', { projectId: project.projectId, ...applyResult });
          const nextCursors = newestCursorsFromChanges(changes, getProjectRecordCursors(project.projectId));
          updateProjectSyncCursors(project.projectId, nextCursors, { source: 'listener-received-changes' });
        } finally {
    ...
    if (token === activeProjectStreamToken) endFirestoreActivity(latestCursorValue(getProjectRecordCursors(project.projectId)));
  }
}
...
function renderProjectList() {
  ...
  list.innerHTML = projects.map((project) => {
    ...
    const deleteButton = project.isSample ? '' : `
      <button type="button" class="project-delete-btn" data-project-delete-id="${escapeHtml(project.projectId)}"
    ...
    return `
      <div class="project-card${active ? ' active' : ''}">
        <button type="button" class="project-card-open" data-project-open-id="${escapeHtml(project.projectId)}" ${active ? 'aria-current="true"' : ''}>
          <strong>${escapeHtml(formatProjectLabel(project))}</strong>${address}
        </button>${deleteButton}
      </div>`;
  }).join('');
}
...
async function deleteProject(projectId) {
  ...
  const scopeText = testProject ? 'このテスト案件を端末とFirestoreから完全に削除します。' : 'この案件をこの端末から削除します。Firestoreの案件データは残ります。';
  ...
  try {
    ...
    try { await deleteLocalPhotoData(photoIds); } catch (error) { console.warn('[v0.1.6.2] 写真キャッシュ削除失敗', error); }
    ...
    window.alert(testProject ? 'テスト案件をFirestoreから削除できませんでした。端末内の案件は残しています。' : '案件を端末から削除できませんでした。');
  }
}
...
async function createProjectFromForm() {
  ...
  try {
    ...
    try { remoteProjectNos = await getRemoteTemporaryProjectNos(dateCode, 'production'); }
    catch (error) { console.warn('[v0.1.6.2] Firestore仮番号確認失敗。端末内番号だけで採番します。', error); }
    const project = createTemporaryProject({ projectName, address, existingProjects: getProjectList(), existingProjectNos: remoteProjectNos });
    const finishRecords = createDefaultFinishRecords();
    saveProjectSnapshot({ project, finishRecords, materialRecords: [], photoRecords: [], source: 'legacy-app-new-project' });
    openProjectSession({ project, finishRecords, materialRecords: [], photoRecords: [] });
...
async function recoverCurrentProjectAfterNetworkReturn() {
  ...
  if (!target) return;
  stopProjectRecordStream();
  try { await openFirestoreProjectSession(target); }
  catch (error) {
