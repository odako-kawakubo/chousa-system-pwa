...
function roomPage(rows) {
  return `<article class="output-paper output-paper-room"><h2 class="output-report-title">部屋別調査対象建材リスト</h2><table class="output-report-table output-room-report"><colgroup><col class="col-floor"><col class="col-room"><col class="col-part"><col class="col-no"><col class="col-name"><col class="col-note"><col class="col-level"><col class="col-result"></colgroup><thead><tr><th>階</th><th>部屋No.</th><th>部位</th><th>建材<br>No.</th><th>建材名称</th><th>備考</th><th>建材<br>レベル</th><th>分析結果</th></tr></thead><tbody>${rows.map((row, index) => {
    ...
    const roomCell = shouldRenderGroupedCell(rows, index, 'roomNo', 'floor') ? `<td class="center group-cell" rowspan="${spanLength(rows, index, 'roomNo', 'floor')}">${esc(row.roomNo)}${row.roomName ? `<div class="output-room-name">${esc(row.roomName)}</div>` : ''}${row.roomNote ? `<div class="output-room-note">${esc(row.roomNote)}</div>` : ''}</td>` : '';
    return `<tr class="${row.registered ? '' : 'is-unregistered'}">${floorCell}${roomCell}<td class="center">${esc(row.part)}</td><td class="center">${esc(row.materialNo)}</td><td>${esc(row.materialName)}</td><td>${esc(row.note)}</td><td class="center">${esc(row.level)}</td><td class="center">${esc(row.analysisResult)}</td></tr>`;
...
function samplingPages(items, photoSources) {
  ...
  return safe.map((item) => {
    ...
    return `<article class="output-paper output-sampling-paper"><h2 class="output-report-title">試料採取写真</h2><table class="output-sampling-meta"><tbody><tr><th>件名：</th><td>${esc(item.projectName)}</td><th>試料：</th><td>${esc(item.projectNo)}${item.projectNo && item.sampleNo ? '-' : ''}${esc(item.sampleNo)}</td><th>採取日：</th><td>${esc(item.capturedDate)}</td></tr><tr><th>場所：</th><td colspan="5">${esc(item.part)}${item.part && item.materialName ? '　' : ''}${esc(item.materialName)}${item.samplingPlace ? `　部屋No.${esc(item.samplingPlace)}` : ''}</td></tr></tbody></table><div class="output-sampling-stages">${['before','during','after'].map((type) => {
      ...
      return `<div class="output-sampling-stage"><div class="output-stage-label">撮影状況：　${esc(stage.label)}</div><div class="output-photo-frame-wrap output-sampling-frame-wrap">${photoFrame(stage.photoId, photoSources, 'output-sampling-frame')}${canSelect ? `<button class="output-photo-expand" type="button" data-output-sampling-expand="${esc(item.materialId)}" data-output-branch="${Number(item.branch)||0}" data-output-stage="${esc(type)}">拡大</button>` : ''}</div></div>`;
    }).join('')}</div></article>`;
  });
}
