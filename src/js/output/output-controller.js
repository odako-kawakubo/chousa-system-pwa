...
function renderMaterialTable(rows) {
  ...
  return `<div class="output-sheet-wrap"><div class="output-sheet-title">調査対象建材リスト</div><table class="output-table output-material-table"><thead><tr><th>建材No.</th><th>建材名称</th><th>部位</th><th>施工範囲</th><th>レベル</th><th>分析の要否</th><th>分析結果</th><th>備考</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${escapeHtml(row.materialNo)}</td><td>${escapeHtml(row.name)}</td><td>${escapeHtml(row.part)}</td><td>${escapeHtml(row.usageLocation)}</td><td>${escapeHtml(row.level)}</td><td>${escapeHtml(row.analysisRequired)}</td><td>${escapeHtml(row.analysisResult)}</td><td>${escapeHtml(row.note)}</td></tr>`).join('')}</tbody></table></div>`;
}

function renderRoomTable(rows) {
  if (!rows.length) return '<div class="output-empty">仕上表レコードはまだありません。</div>';
  return `<div class="output-sheet-wrap"><div class="output-sheet-title">部屋別調査対象建材リスト</div><table class="output-table output-room-table"><thead><tr><th>階</th><th>部屋No.</th><th>部屋名</th><th>部屋備考</th><th>部位</th><th>建材No.</th><th>建材名称</th><th>備考</th><th>レベル</th><th>分析結果</th></tr></thead><tbody>${rows.map((row) => `<tr class="${row.registered ? '' : 'is-unregistered'}"><td>${escapeHtml(row.floor)}</td><td>${escapeHtml(row.roomNo)}</td><td>${escapeHtml(row.roomName)}</td><td>${escapeHtml(row.roomNote)}</td><td>${escapeHtml(row.part)}</td><td>${escapeHtml(row.materialNo)}</td><td>${escapeHtml(row.materialName)}</td><td>${escapeHtml(row.note)}</td><td>${escapeHtml(row.level)}</td><td>${escapeHtml(row.analysisResult)}</td></tr>`).join('')}</tbody></table></div>`;
}

function renderPhotoPlaceholder(title, count, description) {
  return `<div class="output-sheet-wrap output-placeholder-sheet"><div class="output-sheet-title">${escapeHtml(title)}</div><div class="output-placeholder-count">写真レコード ${count}件</div><div class="output-empty">${escapeHtml(description)}<br>帳票レイアウトは次段階で接続します。</div></div>`;
}
