...
function lineBlockHeight(size, count, lineHeight=1.12, paddingY=1.2) {
  return count ? count * size * 0.352778 * lineHeight + paddingY * 2 : 0;
}

function requiredTextHeight(pdf, value, width, options={}) {
  const { size=7.5, lineHeight=1.12, paddingX=.8, paddingY=1.2, roomTokens=false } = options;
  setFont(pdf,size,'normal');
  ...
  return lineBlockHeight(size,lines.length,lineHeight,paddingY);
}
...
function drawSingleLineFit(pdf, value, x, y, width, height, options={}) {
  ...
  const usable = Math.max(1,width - paddingX * 2);
  while (fontSize > minSize && pdf.getTextWidth(text) > usable) {
    fontSize = Math.max(minSize,fontSize - .25);
    setFont(pdf,fontSize,style,color);
  }
...
function footnoteHeight(pdf,settings) {
  ...
  return lines.length * settings.noteSize * .352778 * 1.25;
}
...
function drawMaterialBodyRow(pdf,row,y,rowH,widths,settings) {
  ...
  widths.forEach((width,colIndex) => {
    ...
    drawCellText(pdf,values[colIndex],cx,y,width,rowH,{ align:[0,2,4,5,6].includes(colIndex)?'center':'left',size:settings.materialBodySize,color:positive?RED:BLACK,paddingX:.8,roomTokens:colIndex===3 });
    cx += width;
  });
}
...
function renderMaterialPages(pdf,vm,state,settings) {
  ...
  const bottom=listTableBottom(pdf,settings);
  const pages=paginateRows(rows,heights,bottom-(LIST_START_Y+headerH));
  pages.forEach((page) => {
    addPage(pdf,state);
...
function roomRowBaseHeight(pdf,row,widths,settings) {
  ...
  values.forEach(({col,value})=>{ height=Math.max(height,requiredTextHeight(pdf,value,widths[col],{size:settings.roomBodySize,paddingX:.8,paddingY:.9})); });
  return Math.min(30,height);
}
...
function paginateRoomGroups(rows,heights,availableHeight) {
  ...
  groups.forEach((group)=>{
    ...
    if (groupHeight <= availableHeight) {
      ...
      pageRows.push(...groupRows); pageHeights.push(...groupHeights); used+=groupHeight;
      return;
    }
...
function renderRoomPageBody(pdf,rows,heights,widths,settings) {
  ...
  rows.forEach((row,index)=>{
    ...
    [{col:3,value:row.part,center:true},{col:4,value:row.materialNo,center:true},{col:5,value:row.materialName},{col:6,value:row.note},{col:7,value:row.level,center:true},{col:8,value:row.analysisResult,center:true}].forEach((cell)=>{
      drawRect(pdf,xs[cell.col],y,widths[cell.col],rowH);
      drawCellText(pdf,cell.value,xs[cell.col],y,widths[cell.col],rowH,{align:cell.center?'center':'left',size:settings.roomBodySize,color,paddingX:.8});
    });
    ...
    if(!precedingSame(rows,index,roomKey)){
      ...
      [{col:1,value:row.roomNo,center:true,roomTokens:true},{col:2,value:row.roomName,center:true},{col:9,value:row.roomNote,center:false}].forEach((cell)=>{
        drawRect(pdf,xs[cell.col],y,widths[cell.col],h);
        drawCellText(pdf,cell.value,xs[cell.col],y,widths[cell.col],h,{align:cell.center?'center':'left',size:settings.roomBodySize,color,paddingX:.8,roomTokens:cell.roomTokens});
      });
    }
    y+=rowH;
  });
...
function renderRoomPages(pdf,vm,state,settings) {
  ...
  const bottom=listTableBottom(pdf,settings);
  const pages=paginateRoomGroups(rows,heights,bottom-(LIST_START_Y+settings.roomHeaderHeight));
  pages.forEach((page)=>{
    addPage(pdf,state);
...
function addContainedImage(pdf,dataUrl,x,y,width,height){ if(!dataUrl)return false; try{const props=pdf.getImageProperties(dataUrl);const iw=Number(props?.width||1);const ih=Number(props?.height||1);const scale=Math.min(width/iw,height/ih);const dw=iw*scale;const dh=ih*scale;pdf.addImage(dataUrl,imageFormat(dataUrl),x+(width-dw)/2,y+(height-dh)/2,dw,dh,undefined,'FAST');return true;}catch(error){console.warn('PDF写真配置に失敗しました',error);return false;} }
...
function renderVisualPhotoPages(pdf,vm,photoSources,state,settings){
  const pages=chunkRows(vm.visualPhotoItems||[],VISUAL_ITEMS_PER_PAGE); const left=10; const top=31; const gapX=5; const gapY=4; const slotW=(190-gapX)/2; const slotH=(252-gapY*3)/4; const captionH=settings.visualCaptionLayout==='two-line'?11:8;
  pages.forEach((items)=>{addPage(pdf,state);drawTitle(pdf,'調査対象建材写真帳',settings);for(let index=0;index<VISUAL_ITEMS_PER_PAGE;index+=1){const item=items[index];if(!item)continue;const col=index%2;const row=Math.floor(index/2);const x=left+col*(slotW+gapX);const y=top+row*(slotH+gapY);const photoH=slotH-captionH;const source=photoSources?.get?.(String(item.photoId||''))||'';addContainedImage(pdf,source,x,y,slotW,photoH);renderVisualCaption(pdf,item,x,y+photoH,slotW,captionH,settings);}});
}

function sampleCode(item){ const projectNo=String(item.projectNo??'').trim(); const sampleNo=String(item.sampleNo??'').trim(); const placeCount=String(item.branch??'').trim(); return `${projectNo}${projectNo&&sampleNo?'-':''}${sampleNo}${placeCount?` ${placeCount}`:''}`; }
...
function renderSamplingPages(pdf,vm,photoSources,state,settings){
  ...
  pages.forEach((item)=>{addPage(pdf,state);drawTitle(pdf,'試料採取写真',settings,true);const leftX=10;const metaY=32;const lineH=8.5;const labelW=15;const rightEdge=190;
    ...
    setFont(pdf,settings.samplingMetaSize,'normal');pdf.text(sampleCode(item),rightEdge,metaY,{align:'right'});
    ...
    const stageMap=new Map((item.stages||[]).map((stage)=>[stage.type,stage]));[['before','施工前'],['during','施工中'],['after','施工後']].forEach(([type,label],index)=>{const stage=stageMap.get(type)||{label,photoId:'',memo:''};const y=57+index*72;const photoX=38;const photoW=92;const photoH=69.5;const memoX=133;const memoW=53;const source=photoSources?.get?.(String(stage.photoId||''))||'';addContainedImage(pdf,source,photoX,y,photoW,photoH);drawCellText(pdf,`撮影状況：${stage.label||label}`,memoX,y,memoW,7,{size:settings.samplingStatusSize,style:'bold',maxLines:1,paddingX:0});const memoTop=y+7;const memoH=62;const lh=memoH/9;const memoLines=String(stage.memo||'').split('\n').slice(0,9);for(let line=0;line<9;line+=1){const ly=memoTop+line*lh;pdf.setDrawColor(70,70,70);pdf.setLineDashPattern([.6,.6],0);pdf.line(memoX,ly+lh,memoX+memoW,ly+lh);pdf.setLineDashPattern([],0);drawCellText(pdf,memoLines[line]||'',memoX,ly,memoW,lh,{size:settings.samplingMemoSize,maxLines:1,paddingX:.4});}});
  });
}

function renderTarget(pdf,target,vm,photoSources,state,settings){ if(target==='materials')renderMaterialPages(pdf,vm,state,settings); else if(target==='rooms')renderRoomPages(pdf,vm,state,settings); else if(target==='visual-photos')renderVisualPhotoPages(pdf,vm,photoSources,state,settings); else if(target==='sampling-photos')renderSamplingPages(pdf,vm,photoSources,state,settings); }

async function buildVectorPdf({targets,vm,photoSources,onProgress=null,settings=null}){
  await loadScript(JSPDF_URL,'jspdf'); const {jsPDF}=window.jspdf||{}; if(!jsPDF)throw new Error('PDF生成ライブラリを初期化できませんでした。'); const outputSettings=normalizeOutputSettings(settings||getOutputSettings()); onProgress?.('PDFフォントを準備しています…',.05); const pdf=new jsPDF({orientation:'portrait',unit:'mm',format:'a4',compress:true,putOnlyUsedFonts:true}); await registerFonts(pdf); const state={pageCount:0}; const list=Array.isArray(targets)?targets:[]; list.forEach((target,index)=>{onProgress?.(`PDFを作成中 ${index+1} / ${list.length}`,(index+1)/Math.max(1,list.length));renderTarget(pdf,target,vm,photoSources,state,outputSettings);}); if(!state.pageCount){addPage(pdf,state);setFont(pdf,10,'normal');pdf.text('出力対象がありません。',10,20);} if(pdf.getNumberOfPages()>state.pageCount)pdf.deletePage(1); return pdf;
}

export async function createVectorPdfPreview({targets,vm,photoSources,onProgress=null,settings=null}){ const pdf=await buildVectorPdf({targets,vm,photoSources,onProgress,settings}); return {blob:pdf.output('blob'),pageCount:pdf.getNumberOfPages()}; }
export async function createVectorPdfBlob({targets,vm,photoSources,onProgress=null,settings=null}){ const pdf=await buildVectorPdf({targets,vm,photoSources,onProgress,settings}); return pdf.output('blob'); }
export async function exportVectorPdf({targets,vm,photoSources,filename,onProgress=null,settings=null}){ const pdf=await buildVectorPdf({targets,vm,photoSources,onProgress,settings}); pdf.save(filename); }
