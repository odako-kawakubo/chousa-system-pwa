...
function loadScript(src,globalName) {
  ...
  if(existing)return new Promise((resolve,reject)=>{existing.addEventListener('load',()=>resolve(globalName?window[globalName]:true),{once:true});existing.addEventListener('error',reject,{once:true});});
  return new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=src;script.async=true;script.dataset.outputLib=src;script.onload=()=>resolve(globalName?window[globalName]:true);script.onerror=()=>reject(new Error(`出力ライブラリを読み込めませんでした: ${src}`));document.head.appendChild(script);});
}
function selectedTargets() {
  return OUTPUT_TARGET_ORDER.filter((key)=>modal?.querySelector(`[data-output-target="${key}"]`)?.checked);
}
function setStatus(text,progress=null) {
  const node=modal?.querySelector('[data-output-export-status]');if(node)node.textContent=text||'';
  const bar=modal?.querySelector('[data-output-export-progress]');if(bar&&progress!=null)bar.value=progress;
}
function ensureModal() {
  if(modal)return;
  modal=document.createElement('div');modal.className='output-export-modal';
  modal.innerHTML=`<div class="output-export-card"><div class="output-export-head"><b data-output-export-title>出力</b><button class="btn small" type="button" data-output-export-close>閉じる</button></div><div class="output-export-targets">${OUTPUT_TARGET_ORDER.map((key)=>`<label><input type="checkbox" data-output-target="${key}" checked><span>${esc(OUTPUT_TARGETS[key].label)}</span></label>`).join('')}</div><label class="output-export-combine" data-output-combine-row><input type="checkbox" data-output-combine><span>選択した帳票を1つのPDFにまとめる</span></label><div class="output-export-status" data-output-export-status></div><progress class="output-export-progress" data-output-export-progress max="1" value="0"></progress><div class="output-export-actions"><button class="btn" type="button" data-output-export-close>キャンセル</button><button class="btn primary" type="button" data-output-export-run>実行</button></div></div>`;
  document.body.appendChild(modal);
  modal.addEventListener('click',(event)=>{if(event.target===modal||event.target.closest('[data-output-export-close]')){if(!running)modal.classList.remove('open');return;}if(event.target.closest('[data-output-export-run]'))void runExport();});
}
function openModal(nextMethod) {
  ensureModal();method=nextMethod;
  const title=modal.querySelector('[data-output-export-title]');if(title)title.textContent=method==='pdf'?'PDF出力':method==='print'?'印刷':'Excel出力';
  modal.querySelector('[data-output-combine-row]')?.toggleAttribute('hidden',method!=='pdf');
  modal.querySelector('[data-output-export-progress]').value=0;setStatus('出力する帳票を選択してください。');modal.classList.add('open');
}
async function exportPdf(targets,vm,photoSources,settings) {
  ...
  if(combine){await exportVectorPdf({targets,vm,photoSources,settings,filename:projectFileName('調査資料','pdf'),onProgress});return;}
...
async function exportPrint(targets,vm,photoSources,settings) {
  setStatus('印刷用PDFを作成しています…',.8);
  const blob=await createVectorPdfBlob({targets,vm,photoSources,settings,onProgress:(text,progress)=>setStatus(text,progress)});
  const url=URL.createObjectURL(blob);
  const popup=window.open(url,'_blank');
  if(!popup){URL.revokeObjectURL(url);throw new Error('印刷用PDFを開けませんでした。ポップアップを許可してください。');}
  setStatus('PDFを開きました。PDF画面から印刷してください。',1);
...
function addMaterialSheet(workbook,rows) {
  ...
  sheet.addRow(['建材No.','建材名','部位','施工範囲 / 部屋No.','建材Lv.','分析の要否','石綿含有の有無','調査備考']);styleHeader(sheet.getRow(1));
  ...
  sheet.columns=[{width:10},{width:24},{width:14},{width:28},{width:10},{width:16},{width:18},{width:32}];
}
function addRoomSheet(workbook,rows) {
  const sheet=workbook.addWorksheet('部屋別リスト');
  sheet.addRow(['階','部屋No.','部屋名','部位','建材No.','建材名称','調査備考','建材Lv.','分析結果','部屋備考']);styleHeader(sheet.getRow(1));
...
async function exportExcel(targets,vm,photoSources) {
  ...
  const buffer=await workbook.xlsx.writeBuffer();const header=targets.length===1?OUTPUT_TARGETS[targets[0]].header:'調査資料';downloadBlob(new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),projectFileName(header,'xlsx'));
}
async function runExport() {
  ...
  try{
    ...
    else if(method==='print')await exportPrint(targets,vm,photoSources,settings);
  ...
  }catch(error){console.error('出力に失敗しました',error);setStatus(`出力に失敗しました：${error?.message||error}`);}
  finally{running=false;modal.querySelector('[data-output-export-run]').disabled=false;}
}

export function initializeOutputExportController(root,{getSettings=null}={}) {
  ensureModal();if(typeof getSettings==='function')settingsProvider=getSettings;
  root?.addEventListener('click',(event)=>{const button=event.target.closest('[data-output-export]');if(!button)return;const requested=button.dataset.outputExport;if(!['pdf','print','excel'].includes(requested))return;openModal(requested);});
}
